...
def session_history_to_csv() -> str:
    ...
    writer.writerow(["id", "chat_id", "created_at", "role", "content", "in_progress", "components"])

    for msg in st.session_state.chat_messages:
        # AnalystChatMessage の属性アクセス
        msg_id    = msg.id
        chat_id   = msg.chat_id or ""
        created   = msg.created_at.isoformat() if hasattr(msg, 'created_at') else ""
        role      = msg.role
        content   = msg.content
        in_prog   = msg.in_progress
        # components は JSON 文字列化
        comps     = json.dumps([
            c.dict() if hasattr(c, 'dict') else str(c)
            for c in msg.components
        ],ensure_ascii=False, indent=2, separators=(',', ':'))

        writer.writerow([msg_id, chat_id, created, role, content, in_prog, comps])
...
async def main() -> None:
    ...
    with st.sidebar:
        ...
        with col3:
           st.download_button(
               label="Download Chat",
               data=session_history_to_csv(),
               file_name=f"chat_{st.session_state.current_chat_id or 'session'}.csv",
               mime="text/csv",
               use_container_width=True,
               key=f"download_{st.session_state.current_chat_id or 'session'}"
           )
